...
const estudiantesPorCurso = {
    ...
    "Computación Avanzada": [
        { nombre: "ANA LUCÍA MÉNDEZ", ci: "9876543 LP" }
    ],
    "Salud": [
    ...
    "Orientación Legal": [
        { nombre: "PEDRO ANTONIO GÓMEZ", ci: "1357924 CB" }
    ]
};
...
export default function GeneradorCertificados() {
    ...
    const [formData, setFormData] = useState({
        ...
        firmantes: {
            ...
            cargoSecretario: 'SECRETARIO MUNICIPAL DE EDUCACIÓN Y DESARROLLO SOCIAL GAMLP'
        }
    });
    ...
    const [previewFondo, setPreviewFondo] = useState(fondosPredeterminados.TALLER);
    ...
    const cursos = {
        TALLER: [
            "Computación", "Computación Avanzada", "Manualidades",
            "Repostería y Pastelería", "Inglés Básico", "Danza Folklórica"
        ],
        GESTORIA: [
            "Orientación Legal", "Nuevas Tecnologías",
            "Salud", "Turismo"
        ]
    };
    ...
    const agregarCertificadoAlPDF = (doc, estudiante, pageWidth, pageHeight) => {
        ...
        contenido.split('\n').forEach(linea => {
            if (linea.trim() === '') {
                ...
                if (hasBold) {
                    ...
                    let currentX = (pageWidth - doc.getTextWidth(linea.replace(/<b>|<\/b>/g, ''))) / 2;
                    ...
                    if (linea === estudiante.nombre.toUpperCase()) {
                        ...
                        lines.forEach(line => {
                            doc.text(line, 20, yPos, { align: 'justify', maxWidth: pageWidth - 40 });
                            yPos += 6;
                        });
                    }
                }
        ...
        // Alcalde
        doc.setFont(undefined, 'bold');
        doc.text(formData.firmantes.alcalde, pageWidth / 2, firmaY, { align: 'center' });
        doc.setFont(undefined, 'normal');
        doc.text(formData.firmantes.cargoAlcalde, pageWidth / 2, firmaY + 5, { align: 'center' });

        // Secretario
        doc.setFont(undefined, 'bold');
        doc.text(formData.firmantes.secretario, pageWidth / 2, firmaY + 20, { align: 'center' });
        doc.setFont(undefined, 'normal');
        doc.text(formData.firmantes.cargoSecretario, pageWidth / 2, firmaY + 25, { align: 'center' });
    };
    ...
    const generarPDFCursoCompleto = () => {
        ...
        if (estudiantes.length === 0) {
            alert(`No hay estudiantes registrados para el curso de ${formData.curso}`);
            return;
        }
        ...
        setFormData(prev => {
            ...
            if (['curso', 'fecha', 'añoCurricular'].includes(name)) {
                newData.contenidoPersonalizado = prev.contenidoPersonalizado
                    .replace('[NOMBRE_CURSO]', name === 'curso' ? value : prev.curso || '[NOMBRE_CURSO]')
                    .replace('[FECHA]', name === 'fecha' ? value : prev.fecha || '[FECHA]')
                    .replace('[AÑO]', name === 'añoCurricular' ? value : prev.añoCurricular || '[AÑO]');
            }
    ...
    return (
        <div className="container mx-auto p-6 max-w-6xl">
            <h1 className="text-3xl font-bold text-[#13678A] mb-6">Generador de Certificados UMAM</h1>

            <div className="bg-white p-6 rounded-lg shadow-lg border border-gray-200">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    {/* Columna izquierda - Formulario */}
                    <div className="space-y-6">
                        <h2 className="text-xl font-semibold text-[#012030]">Configuración del Certificado</h2>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-1">Curso/Mención*</label>
                                <select
                                    name="curso"
                                    className="w-full border border-gray-300 rounded-md p-2"
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-1">Año Curricular</label>
                                <input
                                    type="text"
                                    name="añoCurricular"
                                    className="w-full border border-gray-300 rounded-md p-2"
                        ...
                        <div>
                            ...
                            >
                                {formData.fondo !== fondosPredeterminados[formData.tipo] ? 'Cambiar imagen' : 'Seleccionar imagen personalizada...'}
                            </button>
                            {previewFondo && (
                                <div className="mt-2">
                                    ...
                                    <button
                                        onClick={() => {
                                            ...
                                            setFormData(prev => ({ ...prev, fondo: fondosPredeterminados[prev.tipo] }));
                                        }}
                        ...
                        <div className="border-t pt-4">
                            <h3 className="text-lg font-medium text-[#012030] mb-3">Firmantes</h3>

                            <div className="space-y-4">
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 mb-1">Nombre del Alcalde</label>
                                    <input
                                        type="text"
                                        name="alcalde"
                                        className="w-full border border-gray-300 rounded-md p-2"
                                ...
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 mb-1">Cargo del Alcalde</label>
                                    <input
                                        type="text"
                                        name="cargoAlcalde"
                                        className="w-full border border-gray-300 rounded-md p-2"
                                ...
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 mb-1">Nombre del Secretario</label>
                                    <input
                                        type="text"
                                        name="secretario"
                                        className="w-full border border-gray-300 rounded-md p-2"
                                ...
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 mb-1">Cargo del Secretario</label>
                                    <input
                                        type="text"
                    ...
                    <div>
                        <h2 className="text-xl font-semibold text-[#012030] mb-2">Contenido del Certificado</h2>
                        <p className="text-sm text-gray-500 mb-4">
                            Edite el contenido del certificado. Puede usar las siguientes variables:
                            ...
                            <span className="block mt-2">
                                Use <code className="bg-gray-100 px-1">&lt;b&gt;texto&lt;/b&gt;</code> para texto en <b>negrita</b>
                            </span>
                        </p>
                        ...
                        <div className="mt-4 flex justify-between items-center">
                            ...
                            >
                                Generar Certificados ({estudiantesPorCurso[formData.curso]?.length || 0})
                            </button>
                        </div>

                        {formData.curso && estudiantesPorCurso[formData.curso] && (
                            <div className="mt-6 border-t pt-4">
                                <h3 className="text-lg font-medium text-[#012030] mb-2">Estudiantes en este curso</h3>
                                <ul className="max-h-40 overflow-y-auto border rounded-md p-2">
                                    {estudiantesPorCurso[formData.curso].map((estudiante, index) => (
