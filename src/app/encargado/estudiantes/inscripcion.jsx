...
export default function ModalInscripcionAlumno({
  ...
  useEffect(() => {
    ...
    const loadAllData = async () => {
      ...
      try {
        ...
        if (!iRes.ok) throw new Error("Error al cargar inscripciones");

        const [gData, sData, cData, iData] = await Promise.all([
          gRes.json(),
          sRes.json(),
          cRes.json(),
          iRes.json(),
        ...
        // Ordenar gestiones por año descendente y luego por gestión (II antes de I)
        const gestionesOrdenadas = gData.sort((a, b) => {
          if (b.year_id !== a.year_id) {
            return b.year_id - a.year_id; // Año más reciente primero
          }
          // Si es el mismo año, Gestión II antes de I
        ...
        // Para cada inscripción, cargar el horario completo
        const inscripcionesConDatos = await Promise.all(
          iData.map(async (matricula) => {
            try {
              ...
              if (!horarioRes.ok) {
                console.error(`Error cargando horario ${matricula.horario_id}`);
                return null;
              }

              const horarioData = await horarioRes.json();

              // Buscar datos adicionales en lo que ya tenemos cargado
              const curso =
              ...
              const facilitador = usuarioRes.ok ? await usuarioRes.json() : {};
        ...
        // Filtrar nulls
        const inscripcionesValidas = inscripcionesConDatos.filter(
          (i) => i !== null,
        );
        setTodasLasInscripciones(inscripcionesValidas);

        // Detectar gestión actual automática
  ...
  useEffect(() => {
    if (gestionActual && todasLasInscripciones.length > 0) {
      // Filtrar por gestion_id (número)
      const inscripcionesFiltradas = todasLasInscripciones.filter(
        (insc) => insc.gestion_id === gestionActual,
      );
      setInscripcionesExistentes(inscripcionesFiltradas);
    } else {
      setInscripcionesExistentes([]);
    }
